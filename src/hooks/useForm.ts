...
export function useForm<T extends Record<string, any>>({
  ...
  const parseErrors = (result: ReturnType<ZodSchema<T>["safeParse"]>): FormErrors<T> => {
  ...
  const touchAll = useCallback(() => setTouched(buildAllTouched()), [buildAllTouched]);

  const isFormValid = useMemo(() => schema.safeParse(form).success, [schema, form]);
